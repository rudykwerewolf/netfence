...
import (
	...
	"example.com/netfence/internal/db"
...
func main() {
	// значения по умолчанию сразу в переменные (чтобы работало при обходе cobra, когда нет аргументов)
	dbPath := defaultDB
	actor := "root"

	root := &cobra.Command{
		Use:   "netfence",
		Short: "netfence - firewall manager",
	}
	...
	listCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer conn.Close()
			if err := db.ApplyAll(ctx, conn); err != nil {
				return err
			}
	...
	add := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			r := &model.Rule{
				...
				Ports:    prts,
				Enabled:  enabled,
				SrcCIDRs: splitCSV(srcs),
				DstCIDRs: splitCSV(dsts),
			}
			if inif != "" {
				r.InIf = &inif
			}
	...
	// --- Если НЕТ аргументов вообще — сразу TUI ---
	if len(os.Args) == 1 {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
